...
#pyrcc5 -o venv\MAK_icon.py venv\MAK_icon.qrc
...
class Ui_main_window(object):
    ...
    def setupUi(self, main_window):
        ...
        with open("hxls_resource\config\\" + self.config0["配置文件"] + ".json", 'r', encoding='utf-8') as s:
    ...
    def set_home(self):
        ...
        main_window.setWindowTitle(self._translate("main_window", "源库助理"))
    ...
    def choose_add_set_add_page(self):
        ...
        self.choose_dispatch = QtWidgets.QCheckBox(main_window)
        self.choose_review = QtWidgets.QCheckBox(main_window)
        self.choose_market = QtWidgets.QCheckBox(main_window)
        self.choose_recruit = QtWidgets.QCheckBox(main_window)
        ...
        self.set_review = QtWidgets.QPushButton(main_window)
        self.set_market = QtWidgets.QPushButton(main_window)
        self.set_recruit = QtWidgets.QPushButton(main_window)
        ...
        self.market_page = QtWidgets.QWidget()
        self.recruit_page = QtWidgets.QWidget()
        ...
        for num in range(len(trans_list)):
            ...
            strc, strs, strp = "choose_" + fnamep, "set_" + fnamep, fnamep + "_page"
            fc, fs, fp = eval("self." + strc), eval("self." + strs), eval("self." + strp)
            fc.setGeometry(self.ui_zoom(20, 40 + 30 * num, 110, 16))
            fc.setObjectName(strc)
            fc.setText(self._translate("main_window", tname))
            fs.setGeometry(self.ui_zoom(140, 40 + (30 * num), 15, 15))
    ...
    def set_button_connect(self):
        ...
        # 帮助按钮发送帮助文本
        self.helpconfig.clicked.connect(lambda: self.sendhelp("配置切换设置帮助"))
        self.help0.clicked.connect(lambda: self.sendhelp("使用须知"))
        self.help1.clicked.connect(lambda: self.sendhelp("启动设置帮助"))
        self.help2.clicked.connect(lambda: self.sendhelp("作战/重游设置帮助"))
        self.help3.clicked.connect(lambda: self.sendhelp("线下采购设置帮助"))
        self.help4.clicked.connect(lambda: self.sendhelp("战术回顾设置帮助"))
        self.help5.clicked.connect(lambda: self.sendhelp("集市领取设置帮助"))
        self.help6.clicked.connect(lambda: self.sendhelp("舍友访募设置帮助"))
        self.help7.clicked.connect(lambda: self.sendhelp("今日工作设置帮助"))
        self.help8.clicked.connect(lambda: self.sendhelp("使用随机包设置帮助"))
        self.help9.clicked.connect(lambda: self.sendhelp("关闭游戏设置帮助"))
        # 连接开始停止
...
#子页面设置
    ...
    def start_program(self):
        ...
        self.enable1 = QtWidgets.QCheckBox(self.create_page)
        self.daily2 = QtWidgets.QCheckBox(self.create_page)#
        self.time2 = QtWidgets.QDateTimeEdit(self.create_page)
        self.cho_config2 = QtWidgets.QComboBox(self.create_page)
        self.enable2 = QtWidgets.QCheckBox(self.create_page)
        self.daily3 = QtWidgets.QCheckBox(self.create_page)
        self.time3 = QtWidgets.QDateTimeEdit(self.create_page)
        self.cho_config3 = QtWidgets.QComboBox(self.create_page)
        self.enable3 = QtWidgets.QCheckBox(self.create_page)
    ...
    def fight_program(self):
        ...
        self.retour_Label.setText(self._translate("main_window", "重游"))
    ...
    def dispatch_program(self):
        ...
        self.plan1 = QtWidgets.QComboBox(self.dispatch_page)
        self.material2 = QtWidgets.QComboBox(self.dispatch_page)
        self.fund2 = QtWidgets.QComboBox(self.dispatch_page)
        ...
        for num in range(1, 7):
            ...
            fm.setGeometry(self.ui_zoom(0, num*30, 72, 22))
            fm.setObjectName(strm)
            self.comboboxstyle(fm)
            fd.setGeometry(self.ui_zoom(75, num*30, 60, 22))
            fd.setObjectName(strd)
            self.comboboxstyle(fd)
            fp.setGeometry(self.ui_zoom(138, num * 30, 95, 22))
            fp.setObjectName(strp)
...
#任务运行
    ...
    def run(self):
        if not self.run_judge.isChecked():pass
            ...
            self.tlist = [self.config0["游戏启动路径"], nrun_list,fit,drun_list,rv,rt]
...
#配置功能函数
    ...
    def config_rename_finish(self):
        ...
        os.rename("hxls_resource\config\\" + self.old + ".json",
                  "hxls_resource\config\\" + newname + ".json")
    ...
    def create_new_config(self):
        ...
        shutil.copyfile("hxls_resource\默认配置.json", "hxls_resource\config\默认配置" + random_num +".json")
    ...
    def save_config(self):
        ...
        with open(r"hxls_resource\config0.json", 'w', encoding='utf-8') as f:
            json.dump(self.config0, f, ensure_ascii=False,indent=1)
        with open("hxls_resource\config\\"+self.config0["配置文件"]+".json", 'w', encoding='utf-8') as d:
    ...
    def wait_to_time(self):
        while 1:
            ...
            for num in range(1, 4):
                fw, fdt,fe = eval("self.daily" + str(num)), eval("self.time" + str(num)), eval("self.enable" + str(num))
