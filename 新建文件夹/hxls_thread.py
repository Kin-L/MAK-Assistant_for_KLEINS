...
class Thready(QThread):
    ...
    def hxls_start(self):
        imi = imitate("UnityWndClass", "环行旅舍",path = "D:\Program Files (x86)\hxls\环行旅舍.exe")
    ...
    def fight(self):
        ...
        if (not self.tlist[2][0]) and (val==0):
            ...
            wait(1500)
            num = self.tlist[2][1]
            if num==0:x1,y1,x2,y2=519,545,786, 555
            elif num==1:x1,y1,x2,y2=885,554,786, 555
            elif num==2:x1,y1,x2,y2=1248,534,1467,548
            click(x1,y1)
            wait(2000)
            click(x2,y2)
            wait(1500)
    ...
    def getmarket(self):
        ...
        if not findcolor((304, 300, 377, 369), "2000FF", 5)[0]:
            ...
            click(300, 383)
            wait(1000)
            if findcolor((294, 440, 334, 486), "2000FF")[0]:
                click(217,487)
                wait(500)
    ...
    def receive_recruit(self,x,y):
        ...
        if findpic((252, 444, 421, 553), r"hxls_resource\picture\recruit\N.png")[1] >= 0.6:
            ...
            if findpic((252, 444, 421, 553), r"hxls_resource\picture\recruit\SR.png")[1] >= 0.6:
                ...
                self.testsignal.emit("error:舍友访募未知错误。")
                self.accomplish.emit(3)
        click(273, 903)
        wait(1500)
        click(273, 903)
    ...
    def reward(self):
        ...
        if not findcolor((181,474,236,528), "2000FF")[0]:
            ...
            click(159, 490)
            wait(1500)
            if findcolor((753,146,790,189), "2000FF")[0]:
                click(727, 183)
                wait(1000)
                click(1340, 1009)
                wait(2000)
    ...
    def random_gift(self):
        self.testsignal.emit("开始检查：随机礼包。")
        click(144,686)
        wait(2000)
        click(720,301)
        wait(800)
        gdirpath = "hxls_resource\picture\\random_gift"
        ...
        for g in dirlist:
            ...
            while 1:
                ...
                if val >= 0.8:
                    self.testsignal.emit("使用随机礼包："+gname)
                    click(x, y)
                    wait(1000)
                    click(956, 867)
                    wait(1000)
                    ...
                    self.testsignal.emit("暂无随机礼包：。"+gname)
                    break
